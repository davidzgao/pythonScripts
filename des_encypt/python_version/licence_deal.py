...
def test():
	...
	month_time = '6'
	key = "19890915"
        print "start time and duration are : ",start_time ,"|",month_time
	encode_start = desencode(start_time,key)
	encode_long = desencode(month_time,key)
        print "after encode :",encode_start,"|",encode_long
	        #print "encode :",encode_long
	decode_start = desdecode(encode_start,key)
        decode_long = desdecode(encode_long,key)
        print "after decode :",decode_start,"|",decode_long

def print_help():
	print '''\
usage: ./licence_deal.py [Option] [arg_string]
Options:
-e arg	: encode arg_string
-d arg	: decode arg_string
-v	: version
-h	: help
--test	: show test examples
	'''	
...
if __name__  == '__main__':
	if len(sys.argv) < 2:
		...
		if sys.argv[1] == "--test":
			...
			print "Vision 1.0 by davidzgao. DES encode_decode script. "
		else:
			print_help()
	elif len(sys.argv) == 3:
		key = "19890915"
		if sys.argv[1] == "-e":
			...
			encode_str = desencode(encode_str,key)
			print "after DES encode,the encode_string is:\n%s"%encode_str
		elif sys.argv[1] == "-d":
                        decode_str = sys.argv[2]
                        decode_str = desdecode(decode_str,key)
                        print "after DES decode,the decode_string is:\n%s"%decode_str
		else:
			print_help()
	else:
		print_help()
